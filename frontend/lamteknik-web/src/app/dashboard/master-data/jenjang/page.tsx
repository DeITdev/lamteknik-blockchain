...
const jenjangSchema = z.object({
  namaJenjang: z.string().min(1, 'Nama jenjang wajib diisi'),
  kodeJenjang: z.string().optional(),
});

type JenjangFormData = z.infer<typeof jenjangSchema>;
...
export default function JenjangPage() {
  ...
  const { data, loading, error, saving, fetchAll, create, update, remove } = useCrud<Jenjang>(jenjangApi);
  ...
  } = useForm<JenjangFormData>({
    resolver: zodResolver(jenjangSchema),
  });

  const onSubmit = async (formData: JenjangFormData) => {
    try {
      if (editingId) {
  ...
  const handleEdit = (item: Jenjang) => {
    setEditingId(item.id);
    reset({
      namaJenjang: item.namaJenjang,
  ...
  const columns = [
    { key: 'kodeJenjang', label: 'Kode' },
    { key: 'namaJenjang', label: 'Nama Jenjang' },
    {
      key: 'actions',
      label: 'Aksi',
      render: (_: unknown, row: Jenjang) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-secondary-900">Jenjang Pendidikan</h1>
          <p className="text-secondary-500 mt-1">Kelola data jenjang pendidikan</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-secondary-200">
              <h2 className="text-lg font-semibold text-secondary-900">
                {editingId ? 'Edit Jenjang' : 'Tambah Jenjang Baru'}
              </h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
              <FormSection title="Informasi Jenjang">
                <FormGrid cols={1}>
                  <Input
                    label="Kode Jenjang"
                    placeholder="Contoh: S1"
                    {...register('kodeJenjang')}
                  />
                  <Input
                    label="Nama Jenjang"
                    placeholder="Contoh: S1 (Sarjana)"
                    error={errors.namaJenjang?.message}
                    {...register('namaJenjang')}
                  />
                </FormGrid>
              </FormSection>
