...
import { provinsiApi } from '@/lib/api';
import { useCrud, useDebounce } from '@/lib/hooks';

const provinsiSchema = z.object({
  namaProvinsi: z.string().min(1, 'Nama provinsi wajib diisi'),
  kodeProvinsi: z.string().optional(),
});

type ProvinsiFormData = z.infer<typeof provinsiSchema>;
...
export default function ProvinsiPage() {
  ...
  const { data, loading, error, saving, fetchAll, create, update, remove } = useCrud<Provinsi>(provinsiApi);
  ...
  } = useForm<ProvinsiFormData>({
    resolver: zodResolver(provinsiSchema),
  });

  const onSubmit = async (formData: ProvinsiFormData) => {
    try {
      if (editingId) {
  ...
  const handleEdit = (item: Provinsi) => {
    ...
    reset({
      namaProvinsi: item.namaProvinsi,
  ...
  const columns = [
    { key: 'kodeProvinsi', label: 'Kode' },
    { key: 'namaProvinsi', label: 'Nama Provinsi' },
    {
      key: 'actions',
      label: 'Aksi',
      render: (_: unknown, row: Provinsi) => (
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-secondary-900">Data Provinsi</h1>
          <p className="text-secondary-500 mt-1">Kelola data provinsi di Indonesia</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto m-4">
            <div className="flex items-center justify-between px-6 py-4 border-b border-secondary-200">
              <h2 className="text-lg font-semibold text-secondary-900">
                {editingId ? 'Edit Provinsi' : 'Tambah Provinsi Baru'}
              </h2>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
              <FormSection title="Informasi Provinsi">
                <FormGrid cols={1}>
                  <Input
                    label="Kode Provinsi"
                    placeholder="Contoh: 11"
                    {...register('kodeProvinsi')}
                  />
                  <Input
                    label="Nama Provinsi"
                    placeholder="Contoh: DKI Jakarta"
                    error={errors.namaProvinsi?.message}
                    {...register('namaProvinsi')}
                  />
                </FormGrid>
              </FormSection>
